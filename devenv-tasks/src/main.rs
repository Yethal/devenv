        ...
        Command::Run {
            ...
            let tasks: Vec<TaskConfig> = {
                ...
                let data = env::var("DEVENV_TASKS").or_else(|_| {
                    task_file
                        .as_ref()
                        .ok_or_else(|| "No task file specified and DEVENV_TASKS environment variable not set".to_string())
                        .and_then(|path| {
                            fs::read_to_string(path).map_err(|e| {
                                format!("Failed to read {}: {e}", task_source())
                            })
                        })
                })?;
                serde_json::from_str(&data).map_err(|e| {
                    format!("Failed to parse {} as JSON: {e}", task_source())
                })?
            };
            let config = Config {
                tasks,
